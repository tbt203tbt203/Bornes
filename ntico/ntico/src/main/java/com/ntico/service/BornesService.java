...
@Service
public class BornesService {
    private UtilisateursRepository utilisateursRepository;
    private BornesRepository bornesRepository;
    private LogRepository logRepository;
    public static String getCouleurRouge() {
        String bornestyle = "<span style=\"color: red;\">Dès que votre charge est finie merci de déplacer votre voiture ! <br> Ne pas refresh la page plus de 3 fois par minutes !</span>";
        return bornestyle;
    }
    public ArrayList<Utilisateur> getPseudoTableBornes(String pseudo){
        ArrayList<Utilisateur> utilisateurPseudo = utilisateursRepository.findByPseudo(pseudo);
        System.out.println(utilisateurPseudo.get(0).getPseudo());
        return utilisateurPseudo;
    }
    public ArrayList<Utilisateur> getEmailTableBornes(String email){
        ArrayList<Utilisateur> utilisateurEmail = utilisateursRepository.findByEmail(email);
        System.out.println(utilisateurEmail.get(0).getEmail());
            ...
            if (jsonResponse.contains("\"active_charge\":")) {
                ...
                if (activeChargeValue.equals("null")) {
                    ...
                    if (username != null && !username.equals("null")
                        /* + power != null && !power.equals("null")+ energy_wh != null && !energy_wh.equals("null")+ timestamp != null && !timestamp.equals("null")+ start != null && !start.equals("null")*/) {
                        return "<br>&bull; La borne est libre ? <span style=\"color: red;\"> " + isActiveCharge + "</span><br>&bull; Utilisée par : " + username +/* "<br>&bull; Heure du début de charge: " + heureNormaleStr2 + "<br>&bull; Heure actuelle: " + heureNormaleStr+*/"<br>&bull; En charge depuis: "+ hours + " heures, " + minutes + " minutes, " + seconds + " secondes."+ /*+"<br>&bull; La puissance max de la borne est de: "+ energy_wh + " Wh"   +*/  "<br>&bull; La puissance distribuée actuellement est de: "+ power + " Wh" +"<br>&bull; La puissance totale distribuée depuis le début de la charge est de: "+ energy_wh + " Wh" ;
                    } else {
